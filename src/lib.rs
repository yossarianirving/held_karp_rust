...
impl DistanceMatrix {
    ...
    pub fn new(n: usize, distances: Vec<u32>) -> DistanceMatrix {
        let new_array: Vec<Option<u32>> = distances.iter().map(|i|
            match i {
                &0 => None,
                x => Some(*x)
            }).collect();
        ...
        assert_eq!(new_dist.array.len(), new_dist.size * new_dist.size, "Array and size do not match");
        new_dist
    }
...
//   calculate the void column
    let void_col: Vec<(usize, Option<u32>)> =
        (0..w_array.size).map(|i: usize| {
            (0, w_array.get(i, 0))
        }).collect();
    println!("[], {:?}", void_col);
...
//        println!("{:?}", comb_by_len);
        ...
        comb_by_len
            ...
            .for_each(|comb| {
                ...
                (1..w_array.size)
                    ...
                    .for_each(|i| {
                        let vals: Vec<(usize, Option<u32>)> = comb.iter().map(|k| {
                            let w = w_array.get(i, *k);
                            // gets the correct column for A - Vx
                            let d_col: Vec<usize> = comb.iter()
                                .filter(|x| *x != k).cloned().collect();
                        ...
                        let min_val: Option<&(usize, Option<u32>)> = vals.iter()
    ...
    let last_val: (usize, Option<u32>) = *vals.iter()
                        .filter(|i| i.1.is_some())
                        .min_by_key(|i| i.1).unwrap();
